"""Deterministic synthetic profiles used by recovery diagnostics and tests."""
...
_LOG_TAU500 = (-5.0, -4.0, -3.0, -2.0, -1.0, 0.0, 1.0)
_TEMPERATURE_K = (4470.0, 4170.0, 4320.0, 4800.0, 5450.0, 6420.0, 8000.0)


def synthetic_temperature_profile(log_tau500: torch.Tensor) -> torch.Tensor:
    """Interpolate a smooth photospheric test profile in kelvin."""

    x = log_tau500
    xp = x.new_tensor(_LOG_TAU500)
    fp = x.new_tensor(_TEMPERATURE_K)
    flat = x.reshape(-1)
    clamped = flat.clamp(xp[0], xp[-1])
    intervals = xp[1:] - xp[:-1]
    secants = (fp[1:] - fp[:-1]) / intervals
    interior_slopes = (intervals[1:] * secants[:-1] + intervals[:-1] * secants[1:]) / (
    ...
    m0, m1 = slopes[indices - 1], slopes[indices]
    width = x1 - x0

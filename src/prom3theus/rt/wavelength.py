...
def air_to_vacuum_angstrom(wavelength_air_angstrom) -> torch.Tensor:
    ...
    if wavelength.dtype not in (torch.float32, torch.float64):
        raise TypeError(
            "Air/vacuum conversion supports float32 and float64 tensors only."
        )
    ...
    if torch.any((wavelength < 2000.0) | (wavelength > 100000.0)):
        raise ValueError(
            "The adopted standard-air conversion is valid from 2000 to 100000 Angstrom."
        )
    inverse_micrometres_squared = (1.0e4 / wavelength).square()
    refractive_index = (
        1.0
        + 0.00008336624212083
        + 0.02408926869968 / (130.1065924522 - inverse_micrometres_squared)
        + 0.0001599740894897 / (38.92568793293 - inverse_micrometres_squared)
    )

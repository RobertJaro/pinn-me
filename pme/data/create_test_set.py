...
def plot_all_Stokes(atmos):
    ...
    ax[0, 0].plot(atmos.lambda_grid / 1e-10, atmos.I)
    ax[0, 0].set_title("Stokes I")

    ax[0, 1].plot(atmos.lambda_grid / 1e-10, atmos.Q / atmos.I)
    ax[0, 1].set_title("Stokes Q/I")

    ax[1, 0].plot(atmos.lambda_grid / 1e-10, atmos.U / atmos.I)
    ax[1, 0].set_title("Stokes U/I")

    ax[1, 1].plot(atmos.lambda_grid / 1e-10, atmos.V / atmos.I)
...
def create_tstep(time_step):
    ...
    lambda_grid = np.linspace(-.5 * (lambda_end - lambda_start), .5 * (lambda_end - lambda_start), num=n_lambda)
...
if __name__ == '__main__':
    ...
    gUp = 1.5
    gLow = 1.83
    lambda_start = 6300.8 * u.AA
    lambda_step = 0.03 * u.AA
    n_lambda = 50
    b_field_0 = 2000.0
    ...
    for el in range(nTime):
        ...
        pl.savefig(f"/glade/work/rjarolim/data/inversion/test/ch_arr_{el:03d}.png")
        pl.close()

    nameTestCase = "test_set_version_1"
    np.savez(f"/glade/work/rjarolim/data/inversion/{nameTestCase}_{nTime}_spatial_{nx}_{ny}.npz",
             stokes_profiles=stokes_profiles)

    np.savez(f"/glade/work/rjarolim/data/inversion/parameters_{nameTestCase}_{nTime}_spatial_{nx}_{ny}.npz",
             **parameters)

...
def validate_scene_basis(value: torch.Tensor) -> torch.Tensor:
    ...
    orientation = (
        torch.linalg.cross(basis[0], basis[1], dim=0) * basis[2]
    ).sum()
    if orientation <= 0:
...
def direction_to_chart_mm(
    ...
    if torch.any(local[..., 2] <= 0):
        raise ValueError("Directions lie outside the visible hemisphere of the scene chart.")
...
def chart_height_to_position_m(
    ...
    radius = torch.as_tensor(solar_radius_m, dtype=direction.dtype, device=direction.device)
...
def position_to_chart_height(
    ...
    radius = torch.as_tensor(solar_radius_m, dtype=position.dtype, device=position.device)
...
def intersect_sphere_near_side_from_local_point(
    ...
    discriminant = radius.square() - impact.square()
    if torch.any(discriminant < 0):
        raise ValueError("At least one local ray misses the requested spherical surface.")
...
class RayTraceResult:
    ...
    maximum_surface_residual_m: torch.Tensor

...
b = parameter_cube['b_field']
theta = parameter_cube['theta']
chi = parameter_cube['chi']
...
for time_step in range(0, b.shape[2], 1):
    ...
    im = axs[1].imshow(theta[:, :, time_step, 0], cmap='RdBu_r', vmin=0, vmax=np.pi)
    divider = make_axes_locatable(axs[1])
    cax = divider.append_axes("right", size="5%", pad=0.05)
    fig.colorbar(im, cax=cax)
    axs[1].set_title('Theta')
    im = axs[2].imshow(chi[:, :, time_step, 0], cmap='twilight_shifted', vmin=0, vmax=np.pi)

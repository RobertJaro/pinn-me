...
class _FastSynthesizer(nn.Module):
    ...
    def forward(self, atmosphere, wavelength, mu=1.0, radiance_scale=None):
...
def test_dense_lte_evaluation_preserves_raster_and_samples_continuous_depth(
    ...
    assert result["solar_x_mm"].shape == raster.spatial_shape
    assert result["solar_y_mm"].shape == raster.spatial_shape
    np.testing.assert_allclose(result["time_hours"], raster.coords[..., 0])
    np.testing.assert_allclose(result["solar_x_mm"], raster.coords[..., 1])
    np.testing.assert_allclose(result["solar_y_mm"], raster.coords[..., 2])

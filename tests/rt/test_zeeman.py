...
from prom3theus.rt.zeeman import ZeemanPattern, wigner_3j, zeeman_components
...
def test_hinode_line_patterns_obey_selection_rules_and_group_normalization(
    ...
    for delta_m in (-1, 0, 1):
        group = [
            component
            for component in pattern.components
            if component.delta_m == delta_m
        ]
        if group:
            assert sum(component.strength for component in group) == pytest.approx(
                1.0, abs=1e-12
            )
...
def test_group_centers_reproduce_effective_lande_factor():
    ...
    for name in pattern.groups:
        shifts, strengths = pattern.group_tensors(
            name, like=torch.zeros((), dtype=torch.float64)
        )
...
def test_invalid_angular_momentum_parity_is_zero_or_rejected_cleanly():
    # Half-integer coupling parity is a selection rule, not a factorial error.
    assert wigner_3j(0.5, 0.5, 0.5, -0.5, 0.5, 0.0) == 0.0
    with pytest.raises(ValueError, match="integer or half-integer"):
        wigner_3j(0.25, 0.5, 1.0, 0.25, -0.5, 0.25)
    with pytest.raises(ValueError, match="delta J"):
        zeeman_components(0.5, 1.0, 1.0, 1.0)
